"""
Serialization module from velocyto - Pure Python implementation
...
def load_hdf5(filename: str, obj_class: Type[object], ignore_attrs_if_err: List[str] = None) -> object:
    ...
    ignore_attrs_if_err: List[str]
        list of attribute names to skip if loading fails
    ...
    if ignore_attrs_if_err is None:
        ignore_attrs_if_err = []
    ...
    for k in _file.keys():
        try:
            ...
            attr_name = k[1:] if k.startswith("&") else k
            if attr_name in ignore_attrs_if_err:
                # Skip this attribute
                pass
            else:
                raise e
