...
#'
#' @importFrom magrittr %>%
#' @importFrom httr2 req_perform resp_status resp_status_desc resp_body_raw
...
omnipath_httr2_perform <- function(req, path = NULL) {
    ...
    # Perform the request with error handling for API incompatibilities
    resp <- tryCatch({
        # Try with path parameter if provided
        if (!is.null(path)) {
        ...
        # Check for httr2 API incompatibility errors
        error_msg <- conditionMessage(e)
        if (grepl("unused arguments|req_prep|resend_count", error_msg, ignore.case = TRUE)) {
            log_warn(
                'Detected httr2 API incompatibility: %s',
                error_msg
            )
            log_trace('Retrying request without path parameter.')
            # Try alternative call without path if it was the issue
            tryCatch({
                resp_alt <- httr2::req_perform(req)
                # If path was specified, try to save manually
                if (!is.null(path)) {
                    log_trace('Saving response body to path manually: %s', path)
                    writeBin(
                        httr2::resp_body_raw(resp_alt),
                        path
                    )
                }
                return(resp_alt)
                ...
                log_error_with_info(
                    'Failed to perform HTTP request with alternative method.',
                    e2
                )
                stop(e)  # Throw original error
            })
        } else {
            # Not a compatibility issue, re-throw
            stop(e)
        }
    })
